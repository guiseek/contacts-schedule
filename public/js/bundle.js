...
$templateCache.put('./contacts.html','<div class="list-group" ng-if="$ctrl.filteredContacts.length"><a class="list-group-item" ng-repeat="contact in $ctrl.filteredContacts"><contact contact="contact" on-select="$ctrl.goToContact($event);"></contact></a></div><div class="alert alert-warning" ng-if="!$ctrl.filteredContacts.length"><i class="glyphicon glyphicon-alert"></i> N\xE3o tem ningu\xE9m aqui...</div>');}]);})(window.angular);